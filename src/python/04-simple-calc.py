...
for total_loop in range(4):
	...
	for W_index in range(len(W_list)):         # Run loop for W
		...
		if (W_index == 0) or (SE_final > SE): # Set 1st SE to SE_old and SE_old > SE
	...
	for b_index in range(len(b_list)):     # Run loop for b
		...
		if (b_index==0) or (SE_final > SE): # Set 1st SE to SE_old and SE_old > SE
			SE_final = SE                     # Set new SE_final to SE
			b_final = b                       # Set new b_final to b

	# Make W list with incremental value
	W_value = W_final-W_delta*100
	W_list = []
	while W_value <= W_final+W_delta*100:
		W_list.append(W_value)		
		W_value += W_delta

	# Make b list with incremental value
	b_value = b_min-b_delta*100
	b_list = []
	while b_value <= b_max+b_delta*100:
		b_list.append(b_value)
		b_value += b_delta
