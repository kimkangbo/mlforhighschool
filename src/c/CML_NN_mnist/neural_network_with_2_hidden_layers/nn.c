...
///Weights
float Wij[NUM_INPUTS+1][NUM_HIDDEN];
float Wjk[NUM_HIDDEN+1][NUM_OUTPUTS];
...
static float nn_learning(float *x, float *y, float learningrate)
{
	...
	float deltaWij[NUM_INPUTS+1][NUM_HIDDEN]; //adjustments to weights between inputs x and hidden nodes
	float deltaWjk[NUM_HIDDEN+1][NUM_OUTPUTS]; //adjustments to weights between hidden nodes and output y
    ...
    if (debug>=2) nn_debug ("input/hidden weights", (float*)Wij, NUM_INPUTS+1, NUM_HIDDEN);
    ...
    if (debug>=2) nn_debug ("hidden/output weights", (float*)Wjk, NUM_HIDDEN+1, NUM_OUTPUTS);
    ...
    for (h=0; h<NUM_HIDDEN+1; h++)
        for (o=0; o<NUM_OUTPUTS; o++)
            deltaWjk[h][o] = hActivationValues[h] * outputErrors[o];
    if (debug>=2) nn_debug ("hidden/output weights gradient", (float*)deltaWjk, NUM_HIDDEN+1, NUM_OUTPUTS);
